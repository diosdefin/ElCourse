...
const toggleFriend = async (user) => {
  ...
  try {
    const response = await axios.post(
      `${API_BASE_URL}/api/users/${user.username}/friend/`,
      {},
    ...
    console.error('Ошибка переключения дружбы:', error)
    banner.value = {
      type: 'error',
      message: error.response?.data?.detail || 'Не удалось обновить список друзей.',
    }
  } finally {
...
const submitOffer = async () => {
  ...
  try {
    ...
    banner.value = {
      type: 'success',
      message: `Оффер для ${invitedUsername} отправлен.`,
    }
  } catch (error) {
...
onMounted(async () => {
  await Promise.all([fetchSkills(), fetchUsers(), fetchOffers()])
})
        ...
        <div class="flex items-start justify-between gap-4">
          <div class="flex items-center gap-4">
            ...
            <div>
              ...
              <p class="mt-1 text-xs font-semibold uppercase tracking-[0.22em] text-slate-500">
                {{ roleLabel(user.role) }}
              </p>
            </div>
          </div>
