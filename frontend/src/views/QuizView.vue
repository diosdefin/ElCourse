...
<template>
  <div class="mx-auto mt-4 max-w-4xl px-0 pb-12 sm:mt-8 sm:px-4">
    <button
    ...
    <header class="mb-6 rounded-3xl border border-slate-800 bg-slate-900/70 p-5 shadow-2xl shadow-slate-950/20 sm:p-6">
      <p class="text-xs font-bold uppercase tracking-[0.28em] text-indigo-300/80">Итоговое тестирование</p>
      <h1 class="mt-2 break-words text-2xl font-black text-slate-100 sm:text-4xl">Проверка знаний по курсу</h1>
    ...
    <div v-else-if="!result" class="space-y-5">
      ...
      <div
        ...
        :key="q.id"
        class="min-w-0 rounded-3xl border border-slate-800 bg-slate-900/60 p-4 shadow-xl shadow-slate-950/10 sm:p-5"
      >
        <p class="text-xs font-bold uppercase tracking-[0.24em] text-indigo-300">Вопрос {{ index + 1 }}</p>
        <h3 class="mt-2 break-words text-base font-black leading-relaxed text-slate-100 sm:text-lg">{{ q.text }}</h3>

        <div class="mt-5 grid gap-2">
          <label
            v-for="choice in q.choices"
            :key="choice.id"
            class="group flex min-w-0 cursor-pointer items-start gap-3 rounded-xl border px-3 py-3 transition sm:px-4"
      ...
      <div class="sticky bottom-3 rounded-3xl border border-slate-800 bg-slate-950/90 p-3 shadow-2xl shadow-slate-950/40 backdrop-blur sm:bottom-4 sm:p-4">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <button
            class="inline-flex w-full items-center justify-center gap-2 rounded-2xl px-7 py-3 text-sm font-black text-white transition disabled:cursor-not-allowed disabled:opacity-50 sm:w-auto"
    ...
    <section v-else class="rounded-3xl border p-5 text-center shadow-2xl sm:p-8" :class="result.is_passed ? 'border-emerald-400/30 bg-emerald-500/10' : 'border-rose-400/30 bg-rose-500/10'">
      ...
      <h2 class="text-2xl font-black sm:text-3xl" :class="result.is_passed ? 'text-emerald-200' : 'text-rose-200'">
      ...
      <div class="mt-7 grid justify-center gap-3 sm:flex sm:flex-wrap">
        <RouterLink
          v-if="result.is_passed"
          to="/profile"
          class="inline-flex items-center justify-center rounded-2xl bg-emerald-600 px-7 py-3 text-center text-sm font-black text-white transition hover:bg-emerald-500"
        >
          Открыть профиль
        </RouterLink>
        <button
          v-else
          class="inline-flex items-center justify-center rounded-2xl bg-slate-800 px-7 py-3 text-center text-sm font-black text-white transition hover:bg-slate-700"
          @click="resetQuiz"
        >
          Пройти ещё раз
        </button>
        <RouterLink
          ...
          class="inline-flex items-center justify-center rounded-2xl border border-slate-700 bg-slate-950/40 px-7 py-3 text-center text-sm font-black text-slate-300 transition hover:border-indigo-400/60 hover:text-white"
        >
          К описанию курса
        </RouterLink>
      </div>
